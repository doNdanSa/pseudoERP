...
namespace KD
{
    ...
    {
        private OleDbConnection polaczenie = new OleDbConnection(); 
        ...
        private void btn_Zapisz_Click(object sender, EventArgs e)
        {
            ...
            if (txtImie.Text == "" || txtNazwisko.Text == "" || combo_grupa_awarii.Text == "" || comboAwaria.Text == "" || combo_miejsc_awarii.Text == "" || teres_zgl.Text == "")
                ...
                try
                {
                    ...
                    OleDbCommand komendb = new OleDbCommand();
                    OleDbCommand komendc = new OleDbCommand();
                    OleDbCommand komendd = new OleDbCommand();
                    komenda.Connection = polaczenie;
                    komendb.Connection = polaczenie;
                    komendc.Connection = polaczenie;
                    komendd.Connection = polaczenie;
                    ...
                    komendb.CommandText = "insert into Informacja_o_awarii (Grupa_awarii,Awaria,Cisnienie,Mat_gazociagu,Srednica,Awaria_miejscowosc,Awaria_Ulica," +
                    ...
                    komendc.CommandText = "insert into Info_wykonane_prace (Wyl_odb,Ilosc_wyl_odbiorcow,Data_wylaczenia,Data_wlaczenia) values('" + wyl_odb + "','" + txt_wylaczenia.Text + "','" + dateTimePicker5.Text+ "','" + dateTimePicker6.Text + "')";

                    komendd.CommandText = "insert into Sprawca(Typ_sprawcy,Imie,Nazwisko,Nazwa_firmy,Adres,NIP,PESEL,Uwagi) values('" + sprawca + "','" + text_Imie.Text + "','" + text_nazwisko.Text + "','" + text_nazwa_firmy.Text + "'" +
                            ",'" + text_aders_sprawcy.Text + "','" + text_NIP.Text + "','" + text_PESEL.Text + "','" + text_uwagi.Text + "')";

                    
                    komenda.Connection.Open();
                    komenda.ExecuteNonQuery();
                    komendb.ExecuteNonQuery();
                    komendc.ExecuteNonQuery();
                    komendd.ExecuteNonQuery();// insert, delet, update dane w BD
                    komendd.Connection.Close();
